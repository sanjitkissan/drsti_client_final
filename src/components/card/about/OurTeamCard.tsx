  ...
  console.log(typeof(item))
    return (
       ...
       <div className="w-40 h-40 rounded-full overflow:hidden">
         <img src={item.imageurl} alt="imgsanjit kissan" className="w-full h-full  object-cover rounded-full" /> 
       </div>
       <div className="w-full h-full flex flex-col items-center justify-center  gap-2">
       <h1 className="text-2xl text-black hover:text-red-600">{item?.name }</h1>
       {/* <p className="text-black text-lg  ">{item?.content}</p> */}
       <span className='w-full flex items-center justify-center gap-2'>
        <h1 className='text-sm text-black'>{item?.position}</h1>
